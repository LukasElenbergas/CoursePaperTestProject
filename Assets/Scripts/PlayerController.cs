...
public class PlayerController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        //using this for a bigger and more accurate ground check
        RaycastHit2D hit = Physics2D.BoxCast(this.transform.position, new Vector2(0.4f, 0.1f), 0f, Vector2.down, groundCheckRadius, groundMask); 
    ...
    private void Update()
    {
        ...
        //Player dies once he falls into the pit even if he has a powerup, thus we cannot just use Die()
        if (playerRigidbody2D.position.y < deathHeight && !isDead)
        {
    ...
    {
        //Starts slide animation when colides with flagpole
    ...
    void OnCollisionExit2D(Collision2D collision)
    {
        ...
        //Ends slide animation when ends collision with flagpole and starts walking animation
        if(collision.gameObject.tag == "Flagpole") 
        {
            playerAnimator.SetBool("slide", false);
            playerAnimator.SetBool("isEnding", true);
        }
    }
}
